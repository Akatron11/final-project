...
kolon eklemez**. Render Postgres'teki `members` tablosuna `is_inside` kolonu
manuel olarak eklendi (asyncpg ile `ALTER TABLE members ADD COLUMN is_inside
BOOLEAN NOT NULL DEFAULT false;`). Commit `44d77e4` push edildi, Render deploy
oldu, canlıda test edildi (aynı QR art arda → ikinci seferde DENIED_ALREADY_INSIDE). Tamamlandı.

## Bulunan ve Düzeltilen 2. Hata: Eski QR yeniden oluşturma sonrası hâlâ geçerli

**Sorun**: Bir üyeye yeni QR oluşturulduğunda eski QR'ın DB kaydı `is_active=False`
yapılıyordu, ama `/verify`'daki QR sorgusu `credential_value`'yu hiç kontrol
etmiyordu — sadece `member_id + gym_id + type=qr + is_active=True` ile arıyordu.
QR payload'u sadece `gym_id`+`member_id` içerdiği için eski QR decrypt edildiğinde
aynı değerler çıkıyor ve sorgu üyenin **yeni aktif** credential'ını buluyor, eski
QR'ı geçerli kabul ediyordu.

**Düzeltme** (2026-06-11): `app/services/verification.py`'deki QR sorgusuna
`Credential.credential_value == credential_value` koşulu eklendi (NFC tarafında
zaten vardı). Yeni test: `tests/test_verify_decisions.py::test_old_qr_denied_after_regenerating`.
Toplam 15/15 test geçiyor. Henüz commit/push edilmedi.
