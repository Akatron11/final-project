...
The gate device is already authenticated by the time this function is called.
Every scan is logged to access_logs.
The Redis occupancy counter is updated on entry (INCR) and exit (DECR).
"""
...
async def run_verification(body: dict, device: GateDevice, db: AsyncSession) -> dict:
